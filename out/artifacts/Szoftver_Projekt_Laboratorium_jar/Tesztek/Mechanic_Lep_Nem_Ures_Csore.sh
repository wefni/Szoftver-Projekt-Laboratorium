...
cat ../Input/Mechanic_Lep_Nem_Ures_Csore.txt | java -jar ../*.jar > /dev/null 2>&1 & #txt végére mindig kell egy exit

#Jelenlegi teszt
echo "Teszt - Mechanic_Lep_Nem_Ures_Csore"

URES=$(grep -c "pipe-22 ra/re ráléphetnek?  | onComponent.isEmpty(): false" "$FILE")
RALEPETT=$(grep -c "nem tudott rálépni" "$FILE")
if [ $URES -eq 1 ]
then
	if [ $RALEPETT -eq 1 ]
	then
